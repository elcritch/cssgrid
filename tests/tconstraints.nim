...
suite "invalid constraints":
  test "invalid minmax arguments":
    # Test that negative values are rejected
    expect ValueError:
      discard csMin(-100.UiScalar, 1'fr)
    
    # Test that fr units in first argument are rejected
    expect ValueError:
      discard csMin(1'fr, 100.UiScalar)
    
    # Test that negative fr values are rejected
    expect ValueError:
      discard csMin(100.UiScalar, -1'fr)
    
    # Test that zero fr values are rejected
    expect ValueError:
      discard csMin(100.UiScalar, 0'fr)

  test "invalid calc expressions":
    # Test that calc expressions resulting in negative values are rejected
    expect ValueError:
      discard csMin(100.UiScalar - 200.UiScalar, 1'fr)
    
    # Test that calc expressions with invalid units are rejected
    expect ValueError:
      discard csMin(100.UiScalar + 100.UiScalar, 1'fr)
    
    # Test that calc expressions with negative results are rejected
    expect ValueError:
      discard csMax(-100.UiScalar, 1'fr)
